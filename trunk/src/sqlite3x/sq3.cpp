...
namespace sq3 {
	...
	int statement::prepare( char const * sql, int len )
	{
		...
		if( 0 > len ) len = static_cast<int>(strlen(sql));
		sqlite3_stmt * st = 0;
...
#undef STATEMENT_EXECUTE_1ARG_IMPL
	...
	cursor::cursor( cursor const & cp ) : m_stmt(cp.m_stmt),m_cn(0)
	{
		this->copy(cp);
	}
	...
	{
		return this->m_stmt.get()
			? sqlite3_step(this->m_stmt->m_stmt.get())
			: SQLITE_ERROR;
	}
...
#undef CURSOR_GET_STRING_IMPL3
	...
	int database::close( bool force )
	{
		...
		{ // do immediately close:
			return sqlite3_close( this->m_dbh.take() /* transfer ownership to sqlite3 */ );
		}
		else
...
// 		int database::execute(std::string const & sql, std::wstring & tgt);
	...
	int transaction::begin()
	{
		...
		int rc = this->m_db.execute("begin");
